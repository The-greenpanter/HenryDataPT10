TRUNCATE TABLE alumno; -- Vaciar los registros
...
/* El área de Learning le solicita un listado de alumnos de la 
Cohorte N°1243 que incluya la fecha de ingreso.*/


CREATE VIEW alumno_1243 AS
...
/* Como parte de un programa de actualización, el área de People le solicita un listado 
de los instructores que dictan la carrera de Full Stack Developer.*/

SELECT i.idInstructor ,i.cedulaIdentidad, CONCAT(i.nombre," ",i.apellido) AS nombre_completo,c.codigo
FROM instructor i
JOIN cohorte c
ON i.idInstructor = c.idInstructor
WHERE c.codigo LIKE "FT%";                  -- INNER
...
CREATE VIEW alumno_1235 AS
SELECT idAlumno, CONCAT(nombre," " ,apellido) AS nombre_completo, cedulaIdentidad 
FROM alumno
...
b. ¿Que tipo de relación existe entre la tabla cohortes y alumnos? 1:M
...
d. ¿Proponga dos opciones para filtrar el listado solo por los alumnos que pertenecen a 'Full Stack Developer', utilizando " = " y " != "? ¿Cual de las dos opciones es la manera correcta de hacerlo?, ¿Por que? e. ¿Como emplearía el filtrado utilizando el campo idCarrera?*/
